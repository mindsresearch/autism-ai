...
y = [list(df[df['FILE_ID'] == x[0]]['DX_GROUP'])[0]-1 for x in tqdm(data, desc='pain')]

X = [x[1].astype(np.int8) for x in data]
...
model.fit(x=X_tr, y=y_tr, epochs=50, batch_size=256, validation_data=(X_dv,y_dv), verbose=1)
model.save('cnn_1')
